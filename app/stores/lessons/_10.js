var _10 = {
  title: "Arrays and Lab",
  description: "In this lesson, we will go over functions more in depth and create a cash register app. If we have additional time we will quickly go over Arrays and you will be given more time to create your Weather App.",
  slides: "/10-functions-and-arrays.md",
  number: "10",
  date: '03.23.16',
  Homework: [
    {
      title: "Week 5 Assignment",
      url: "https://github.com/ga-students/FEWD-SEA-8/tree/master/Week_05_JS_ComputerScience/Assignment/starter_code",
      urlText: "CitiPix",
      description: "You will be building a web-app that will take a user's input and update the background image of the page based on that input."
    }
  ],
  Supplemental: [
    {
      title: "Coding",
      url: "https://thimble.mozilla.org/",
      urlText: "Thimble",
      description: "Thimble is a site to help you learn HTML, CSS, and JavaScript. It was craeted by Mozilla, who is a great resource for learning."
    }
  ]
};

module.exports = _10;